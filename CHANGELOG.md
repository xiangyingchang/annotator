...
**让保存后的矩形/箭头标记更显眼**：之前标记的 fill 仅 8% 透明度、stroke 2px，数字 "1" 用 14px 橙字白描边，在浅色卡片上几乎看不清。

- `.mark-shape` fill 从 `rgba(255,87,34,0.08)` 升到 `0.18`，stroke 从 2px 升到 3px
- `.mark-num` 改为白字 + 4px 橙描边（`stroke-linejoin: round`），从 14px 升到 16px，字重 600→700
- 新增 `markPulse` 1.2s ×2 动画：保存后新标记的 stroke / fill 透明度脉冲 2 次，让用户肉眼能跟上"刚保存的标记落在哪"
- header 版本号 `v2.9` 同步为 `v3.2`（之前漏改，导致用户以为自己在旧版本上）
